...
**Last synced:** 2026-07-21 (new finding: Open Notify roster ~18 months stale — headcount-plausibility stopgap added)
...
| — | Open Notify roster ~18 months stale | **Fixed, not deployed** (client-side only — will read **Fixed and live** automatically on merge, no Worker deploy needed) | PR #109 (`feat/crew-plausibility-check`), open | Not one of the original 37 findings. Verified directly against NASA's current expedition rosters and Wikipedia (not assumed): Open Notify's astros.json was serving a crew roster roughly 18 months out of date. Headcounts happened to look plausible even though the specific names were stale, so neither F1b's ok:false handling nor F3's stale-selection guard could have caught it — this incident was invisible to every existing safeguard. Added a partial stopgap, not a full fix: `fetchAndRenderCrew()` (apps/web/src/ui/crew.js) now compares Open Notify's headcount against how many seats are actually docked at the selected station right now, via `state.capsulesData` (populated every live sync by live.js's `fetchCapsuleStatus()`) and a new per-family seat table, `CREW_SEATS_BY_FAMILY` (packages/catalog/src/classify.js: soyuz 3, dragon 4, shenzhou 3 — families absent from the map are treated as unknown/unverifiable, never guessed at zero). Flags only gross mismatches (no vehicle docked yet crew reported; crew reported as zero despite a docked vehicle; an undercount beyond a 1-person tolerance) as a small "Roster may not reflect the current crew" caveat under the still-fully-rendered roster — never flags an overcount (handover overlaps between two docked vehicles are normal) and never flags purely because a docked vehicle's family isn't in the seat table. **This check explicitly would NOT have caught tonight's actual incident** — the stale roster's headcount was plausible, only the names were wrong, which this check has no way to verify (capsule-status.json carries vehicle telemetry, never astronaut names). Replacing Open Notify as the crew data source entirely is the real fix and is tracked separately as a larger follow-up (see the Open Questions row on crew data reliability below) — this stopgap narrows the blast radius in the meantime, it doesn't replace that work. Client-side only (`packages/catalog` + `apps/web`) — no Worker changes, auto-deploys to web on merge. |
...
| Crew data reliability — Open Notify (`astros.json`) confirmed down in production 2026-07-16; should a committed-JSON fallback (same pattern as `/today`/`/capsules`, refreshed by a scheduled workflow) be built so crew names survive future outages? | Raised 2026-07-16. Not built yet — no live data was available this session to seed or validate the pipeline end-to-end. This session's fix (PR #96, merged) only stops the Worker from caching a failed fetch for the full hour and gives an honest "temporarily unavailable" message instead of implying zero crew — but that fix itself is **not live yet**, blocked on the Worker deploy (see change log — no Cloudflare credentials in this sandboxed session). **Update 2026-07-21:** Open Notify's failure mode turned out to be broader than "down" — it can also serve a roster that responds fine but is badly stale (found ~18 months out of date; see the new "Open Notify roster ~18 months stale" row above). A headcount-plausibility stopgap is in place, but it can't verify names, only gross counts — replacing Open Notify as the data source entirely remains the real fix and is still undecided/not started. |
