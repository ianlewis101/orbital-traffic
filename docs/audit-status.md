...
| F14 | Per-user direct SATCAT fetch | **Fixed, not deployed** | PR (branch `claude/satcat-via-worker-uw51zr`), open | Worker gets a new `/satcat?id=` route (`fetchSatcat()`, `SATCAT_TTL` = 7 days — this data is effectively permanent once an object is catalogued) following the exact `cached()`/per-key pattern `/passes` already uses. `enrichSatcat()` in `apps/web/src/ui/info.js` now fetches `${WORKER_BASE}/satcat?id=` instead of `celestrak.org` directly; response shape changed from CelesTrak's raw array to a single record-or-null (client logic simplified accordingly, no behavior change). Requires a Worker deploy per Critical Rule #1 once merged — client-side change alone won't take effect without it, since the fetch URL points at the Worker. |
